...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def _initialize_policy(
        ...
        # If dynamic graph is enabled, set separate learning rate for dynamic graph parameters
        if getattr(self.config.MODEL, 'use_dynamic_graph', False):
            dynamic_graph_lr = getattr(self.config.MODEL, 'dynamic_graph_lr', self.config.IL.lr)
            
            # Separate dynamic graph parameters from other parameters
            dynamic_graph_params = []
            other_params = []
            
            for name, param in self.policy.named_parameters():
                if 'w1' in name or 'w2' in name or 'w3' in name or \
                   'semantic_sim_mlp' in name or 'instruction_rel_mlp' in name or \
                   'dist_embed' in name:  # Distance embedding layer also belongs to dynamic graph parameters
                    dynamic_graph_params.append(param)
                else:
                    other_params.append(param)
            
            # Create parameter groups, set different learning rate for dynamic graph parameters
            param_groups = [
                {'params': other_params, 'lr': self.config.IL.lr},
            ]
            if dynamic_graph_params:
                param_groups.append({
    ...
    def _eval_checkpoint(
        ...
        while envs.num_envs > 0 and len(stats_episodes) < episodes_to_eval:
            ...
            for i in range(envs.num_envs):
                if next_episodes[i].episode_id in stats_episodes:  # Duplicate episode appears, indicating all episodes in this simulator have been processed
